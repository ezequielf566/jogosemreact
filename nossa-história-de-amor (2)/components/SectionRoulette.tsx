...
import { 
  ...
  ChevronDown, ChevronUp, Mail, HeartHandshake, Send
} from 'lucide-react';
...
const SectionRoulette: React.FC = () => {
  ...
  const spinRoulette = () => {
    ...
    setTimeout(() => {
      ...
      confetti({
        ...
        spread: 70,
        origin: { y: 0.6 },
        colors: ['#f43f5e', '#ec4899', '#a855f7', '#ffffff'],
  ...
  return (
    <section className="h-[100dvh] w-full bg-transparent flex flex-col items-center justify-center py-4 md:py-10 px-4 overflow-hidden relative">
      ...
      <motion.div 
        ...
        className="z-10 text-center mb-2 md:mb-6"
      >
        <h2 className="text-2xl md:text-5xl font-serif text-white mb-1 md:mb-4 flex items-center justify-center gap-2 md:gap-3 drop-shadow-[0_0_15px_rgba(244,63,94,0.5)]">
          <Sparkles className="text-rose-400 w-4 h-4 md:w-8 md:h-8" />
          Roleta do Casal
          <Sparkles className="text-rose-400 w-4 h-4 md:w-8 md:h-8" />
        </h2>
        <p className="text-zinc-300 text-xs md:text-lg font-medium">Não sabem o que fazer hoje? Deixe a sorte decidir!</p>
      </motion.div>
      ...
      <motion.div 
        ...
        className="z-20 mb-4 md:mb-8 flex flex-wrap justify-center gap-1 md:gap-2 max-w-2xl"
      >
        {THEMES.map(theme => (
          <button
            key={theme.id}
            onClick={() => handleThemeChange(theme.id)}
            className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[10px] md:text-xs font-bold transition-all border-2 ${
              currentThemeId === theme.id 
                ? 'bg-rose-500 text-white border-rose-400 shadow-lg shadow-rose-500/40 scale-105' 
        ...
        <button
          ...
          className="px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[10px] md:text-xs font-bold bg-zinc-800/80 text-zinc-100 border-2 border-zinc-700 hover:bg-zinc-700 transition-all flex items-center gap-1.5 md:gap-2"
        >
      ...
      <motion.div 
        ...
        className="relative w-[220px] h-[220px] sm:w-[280px] sm:h-[280px] md:w-[400px] md:h-[400px] mb-4 md:mb-10 z-10"
      >
      ...
      <motion.button 
        ...
        disabled={isSpinning}
        className={`z-10 px-5 py-2.5 md:px-8 md:py-4 rounded-full font-bold text-sm md:text-lg tracking-wide transition-all duration-300 shadow-xl border-2 ${
          isSpinning 
            ? 'bg-zinc-800 text-zinc-500 border-zinc-700 cursor-not-allowed' 
            : 'bg-gradient-to-r from-rose-500 via-rose-600 to-purple-600 text-white border-rose-400 hover:scale-105 hover:shadow-rose-500/40 active:scale-95'
        }`}
      >
      ...
      {/* Result Display */}
      <div className="h-16 md:h-24 mt-2 md:mt-8 z-10 flex items-center justify-center w-full">
        {selectedOption && !isSpinning && (
          <motion.div 
            initial={{ opacity: 0, y: 10, scale: 0.8 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            className="text-center bg-zinc-900/80 backdrop-blur-sm px-5 py-2 md:px-8 md:py-4 rounded-2xl border border-zinc-800"
          >
            <p className="text-zinc-400 text-[9px] md:text-sm mb-0.5 md:mb-2 uppercase tracking-widest">O destino escolheu:</p>
            <h3 className="text-lg md:text-3xl font-serif text-white flex items-center justify-center gap-2 md:gap-3">
              <selectedOption.icon className="text-rose-400 w-5 h-5 md:w-7 md:h-7" />
              {selectedOption.text}
            </h3>
          </motion.div>
        )}
